...
(defun make-flat-dirs-list (dir &optional reverse-order-p &key return-namestrings-p
                                (recursion-limit 10) return-flat-filelists-p)
  "In U-Files.lisp, will take dir tree with scattered files at all levels. It separates files IN EACH DIR INTO OWN LIST FOR EACH DIR-BRANCH in the file-tree.  RETURNS (values flat-dirs-paths flat-dirs-filenames flat-file-lists  all-flat-filenames)  Returns flat-dirs-paths (path OBJECTS)= a flat list of all these DIRS (each with own FILES-only list) irrespective of their level in the dir tree. "
  (setf *ufiles-out nil
        n 0)
...
;; (afout '*ufiles-out (format nil "In make-flat-dirs-list, FINAL, flat-dirs-paths= ~A~% sub-dir-list= ~A~% ~% simple-all-files-list= ~A~%"flat-dirs-paths sub-dir-list simple-all-files-list ))
  ...
  ;;  (fout *ufiles-out)
    ...
    (when return-flat-filelists-p
      (setf flat-file-lists (flatten-1-level flat-dirs-paths)))
    ...
    (when (and return-namestrings-p return-flat-filelists-p)
      (setf all-flat-filenames (namestrings-list flat-file-lists)))
    (values flat-dirs-paths flat-dirs-filenames
          flat-file-lists  all-flat-filenames) 
    ;;end let, make-flat-dirs-list
    ))
;;TEST
;;  (make-flat-dirs-list  "C:\\3-TS\\LISP PROJECTS TS\\h-help" NIL return-flat-dirpaths-pT)
...
(defun list-all-drives (&key (drive-letters '(a b C d e f g h i j k l m n o p q r s t u))
                             ...
                             (save-to-file-p T)(data-dir-location 
                                                (cond ((boundp '*tomex-data-dir)*tomex-data-dir)
  ...
  (setf drive-sym-boundp (boundp 'drive-syms-sym))
    ;;(break)
    (loop
     ...
     (let
       ...
       (when (directory-p path)
         ...
         (multiple-value-bind (cur-dir host volname serial-num n-dirs n-files 
           ...
           (when (or  (and drive-syms-sym drive-sym-boundp
...
;; modified 2019
;;ddd
(defun file-pathname-p (path &key (type-end 6) return-details-p)
  "In U-files.lisp, Returns a file path string name from either real path or string name.
 If RETURN-DETAILS-P, RETURNS (values filename filetype dir) if filename at end.
Note: If filename or filetype= *, returns :WILD."
  (let*
      ((dptest-p (directory-pathname-p path))
        (len-path) 
        (file-pathname)
        (start2) 
        )
    (unless dptest-p
      (setf len-path (length path)
            start2 (- len-path type-end))
      (when (and (>= start2  0)
                 (search "." path :start2 start2))
        (multiple-value-bind (dir filename filetype)
            (get-file-dir-name-type path)
          (values filename filetype dir)
          ))
      ;;end unless
      )
    ;;end let, file-pathname-p
    ))
;;TEST
...
;;  (file-exists-p "c:\\tomex\\saved\\FO5-64.lisp") = 
;; works= #P"c:/tomex/saved/FO5-64.lisp"  "FO5-64.lisp"
...
;;  (probe-file "c:\\tomex\\saved\\FO5-64.lisp") = NIL
;; (let ((path1 (pathname "c:\\tomex\\saved\\FO5-64.lisp"))) (probe-file  path1)) = NIL
     ...
     (let*
       ...
       (multiple-value-bind (tested-dir filename filetype pathobject new-dir)
           (my-ensure-path-exists pathname :host host
                                     ...
                                     :ensure-pathobject-exists-p ensure-pathobjects-exists-p)
...
(defun my-ensure-path-exists (pathname &key  host (return-filename-p T)
                                             add-filename-end checked-dirs
                                            (chk-host-exist-p T) (slashstr "\\") ensure-pathobject-exists-p)
  ...
  ;;SSSSS USE ENSURE-DIRS-EXIST  HERE (SIMPLER) FOR DIRS
  ;; FINISH REST 
  (setf pathname (namestring pathname))
  (let
      ((new-dir)
         (dirpath (my-directory-namestring pathname))
         (file-path-p (file-exists-p pathname))
         (host)
    ...
    ;;If path ends in a filename make newfilename 
    (when filename
      (setf filetype (pathname-type pathname))
      (cond
       (add-filename-end
    ...
    ;;host
    (unless host
      (setf host (host-namestring pathname)))
    ;;fulldirpath
    (setf fulldirpath (format nil "~A:~A" host dirpath))
       ...
       (when chk-host-exist-p
         (setf host-exists-p (drive-exists-p host)))
          ...
          (when host-exists-p
            ...
            ;;directory-p
            (setf fulldirpath (ensure-final-string fulldirpath :final-str slashstr))
            (multiple-value-bind (dirpath1 dir-list)
                (directory-p  fulldirpath)
              (cond
               (dirpath1
                (setf dir-exists fulldirpath))
               (t (make-directory fulldirpath)))

              ;;CHANGE BELOW
              (when return-filename-p
                (setf filename (pathname-name pathname)
                      filetype (pathname-type pathname)))
              ;;(break "0")
              ;;end when,mvb,when
              ))
    ...
    (when ensure-pathobject-exists-p
      (setf pathobject (pathname pathname)))
     (values dir-exists filename filetype pathobject) 
    ;;end let, my-ensure-path-exists
    ))
;;TEST MAKING NEW DIR TOO  SSSSS START HERE TESTING
;;
;;TEST
;; (my-ensure-path-exists "F:\\3-TS\\2 TEST-DIR" :ensure-pathobject-exists-p T)
;; (my-ensure-path-exists "C:\\3-TS\\LISP PROJECTS TS\\Math\\Permutations\\readme.lisp" :ensure-pathobject-exists-p T)
...
;; (my-ensure-path-exists **testpth2  :ensure-pathobject-exists-p T))
;; works= "C:\\3-TS\\LISP PROJECTS TS\\Math\\Permutations\\"    "readme"  "lisp" #P"C:/3-TS/LISP PROJECTS TS/Math/Permutations/readme.lisp"  NIL
;;IF PREVIOUSLY TESTED
;; (my-ensure-path-exists **testpth2  :ensure-pathobject-exists-p T :checked-dirs '("C:\\3-TS\\LISP PROJECTS TS\\Math\\Permutations\\")  :ensure-pathobject-exists-p T)
;; works= "C:\\3-TS\\LISP PROJECTS TS\\Math\\Permutations\\"  "readme"  "lisp"  #P"C:/3-TS/LISP PROJECTS TS/Math/Permutations/readme.lisp"  NIL



;;MY-ENSURE-DIRS-EXIST
;;2019
;;ddd
;;NAME
;;2019
;;ddd
(defun my-ensure-dirs-exist (path  &key (chk-is-filepath-p T)  )
  ...
  (let
    ...
    (values result path1)
    ;;end let, my-ensure-dirs-exist
    ))
;;TEST
;; (my-ensure-dirs-exist "F:\\1 NEW3\\NEWX")
;; works= "F:\\1 NEW3\\NEWX/"  "F:\\1 NEW3\\NEWX/"
;; ;; (my-ensure-dirs-exist "F:\\1 NEW4\\NEWX\\testfile.txt")
...
(defun ensure-save-file-to-dir (from-filepath to-path)
  ...
  (let
      ((dir-exists (my-ensure-path-exists to-path))
...
;;2017
;;ddd
(defun make-directory (dirpath &key (show-results-p T)(default-end-slash "\\"))
  ...
  (let
      ((dirpath1)  ;;(filename-no-ext "this.txt")
    ...
    (multiple-value-bind (dirpath2 dir-created-p)
      ...
      ;;SSSSS START HERE, FIX ENSURE- FUNCS ETC & THIS TO MAKE DIRS
     ;; WHETHER END IN FILE, NO-SLASH, OR ANY SLASH.
     ;; CL ensure-directories-exist WORKS FOR MULTI-DIRS IF ENDS IN SLASH
...
;; (ensure-directories-exist "c:/temp-silly/") 
;; works= "c:/temp-silly/"  T [created dir]
...
(defun get-file-dir-name-type (filepath)
  "U-files  RETURNS (values dir filename filetype host).INPUT: string or path."
  (let
      ((dir (directory-namestring filepath))
       (filename (pathname-name filepath))
       (filetype (pathname-type filepath))
       (host (pathname-host filepath))
       )
    (values dir filename filetype host)
    ;;end let, get-file-dir-name-type
    ))
;;TEST
;; (get-file-dir-name-type "C:\\this\\that\\other\\testfile.txt")
;; works= "\\this\\that\\other\\"  "testfile"  "txt"  "C"
;; for path object
...
(defun my-directory-namestring (pathstring &key incl-host-p host replace-host-p
  ...
  RETURNS (incl-hosto-p (values host-dir-namestr dir-namestring))   (t (values dir-namestring host-dir-namestr))). If no host, returns NIL for host-dir-namestr"
  (let
      ((dir-namestring)
    ...
    (cond
     (incl-host-p 
      (values host-dir-namestr dir-namestring))
     (t (values dir-namestring host-dir-namestr)))
    ;;end let, defun
    ))
;;TEST
...
;;2019
;;ddd
(defun ensure-end-slash (string)
  "U-files,  Ends string with foreward slash. Uses win32::check-trailing-slash"
  (let
      ((newstring (win32::check-trailing-slash string))
       )
    newstring
    ))
